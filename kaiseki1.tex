...
\begin{frame}
  ...
  =& 𝒪(η ^2)
  \end{align*}
  \uncover<2->{微分積分の\alert{線形性}から先程の雑なオーダーが使えている事がわかる}
\end{frame}
...
\begin{frame}
  ...
  \begin{equation}
    \sum_{i=0}^{n} (-1)^i \diff**[i]t{ \diffp{L}{ \diff[i]{q}{t}}[]} =
  \end{equation}
\end{frame}
...
\begin{frame}{エネルギーが保存する条件}
  \begin{align*}
    ...
    \uncover<3->{=& \sum_{i=1}^{N} \dot{q}_i  \Bigg(\underbrace{\diff**{t}{\diffp{L(q(t), \dot{q}(t),t)}{ \dot{q}(t)}} - \diffp{L(q(t), \dot{q}(t),t)}{q(t)}}_{\text{Euler-Lagrange equation}}\Bigg)- \diffp{L}{t}\\ }
    \uncover<4->{=& -\diffp{L}{t}}
  \end{align*}
  \uncover<5->{ラグランジアンが\alert{陽に}時間依存しないときエネルギーは保存される。}
\end{frame}
...
\begin{frame}{ある座標に陽に依存しない場合の保存量}
  ...
  \end{equation} 
  \uncover<2->{ラグランジアンがある座標 \( q_i \)に陽に依存しないとき、対応する \( p_i \)（\alert{循環座標}）が保存される。 } 
\end{frame}
...
\begin{frame}{ラグランジアンの存在条件}
  \begin{block}{ヘルムホルツ条件}
    微分方程式 \( \mathcal{D}_i(q, \dot{q}, \ddot{q},t) =0 \quad (i=1,2, \ldots, N)\)が与えられたとき、
    ...
    \begin{gather}
      ...
      \diffp{\mathcal{D}_i}{q_j} - \diffp{\mathcal{D}_j}{q_i}= \frac{1}{2} \diff**t{ \diffp{\mathcal{D}_i}{ \dot{q}_j}-\diffp{\mathcal{D}_j}{ \dot{q}_i}[]} 
    \end{gather}
  \end{block}
\end{frame}
\end{document}