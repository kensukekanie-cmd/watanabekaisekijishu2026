...
\begin{teiri}{オイラー・ラグランジュ方程式（Euler-Lagrange equation）}
...
\begin{align*}
  ...
  =&\int_{t_i}^{t_f} \dl{t} \, ε
    ...
    \Biggr]+ 𝒪(ε ^2)
\end{align*}
\footnote{あんまりに煩雑だったので、時間微分のドットを \( i \)階微分のとき \( \overset{i}{q}\)  のように表した。表記ブレブレでごめんね}
\section{一般のEuler-Lagrange equation}
最初に、 \( L(q, \dot{q}, \ddot{q},t) \)について考えてその後一般化する。
\begin{align*}
  δS
  =&\int_{t_i}^{t_f} \dl{t}
    \left( 
  ...
  =&\int_{t_i}^{t_f} \dl{t} 
    \left( 
     ...
     \right) + ε\diffp{L}{\dot{q}}\bigg|_{t_i}^{t_f} + \dot{ε} \diffp{L}{\ddot{q}}\bigg|_{t_i}^{t_f} + 𝒪(ε^2)\\
  =&\int_{t_i}^{t_f} \dl{t} ε
    \left( 
      \diffp{L}{q}- \diff**t{\diffp{L}{\dot{q}}[]}+\diff**[2]t{\diffp{L}{\ddot{q}}[]} 
     \right)+ ε\diffp{L}{\dot{q}}\bigg|_{t_i}^{t_f}
      + \dot{ε} \diffp{L}{\ddot{q}}\bigg|_{t_i}^{t_f} - ε \diff**t{\diffp{L}{\ddot{q}}[]} \bigg|_{t_i}^{t_f}+𝒪(ε^2)
\end{align*}

\begin{teiri}{Euler-Lagrange equation 引数 \( (q, \dot{q}, \ddot{q},t) \) }
  \( ε(t_i)=ε(t_f)=0 ,\dot{ε}(t_i)= \dot{ε}(t_f)=0 \)となる変分に対して作用 \( S[q] \)が停留となる条件は
  \begin{equation}
    \diffp{L}{q}- \diff**t{\diffp{L}{\dot{q}}[]}+\diff**[2]t{\diffp{L}{\ddot{q}}[]}=0 %\quad (\forall i=1,2, \ldots, N)
  \end{equation} 
\end{teiri} 

次に、 \( L(q, \dot{q}, \ldots, \overset{n}{q} ,t) \)について考える。
...
\begin{teiri}{一般のEuler-Lagrange equation}
   \( ε(t_i)=ε(t_f)=0 , \dot{ε}(t_i)= \dot{ε}(t_f)=0 ,\ldots, \overset{n}{ε}(t_i)=\overset{n}{ε}(t_f)=0  \)
